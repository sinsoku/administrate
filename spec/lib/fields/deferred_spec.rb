...
describe Administrate::Field::Deferred do
  ...
  describe "#sortable?" do
    context "when given a `sortable` option" do
      it "returns the value given" do
        sortable_deferred = Administrate::Field::Deferred.
          new(double(sortable?: false), sortable: true)
        unsortable_deferred = Administrate::Field::Deferred.
          new(double(sortable?: true), sortable: false)
    ...
    context "when not given a `sortable` option" do
      it "falls back to the default of the deferred class" do
        sortable_deferred = Administrate::Field::Deferred.
          new(double(sortable?: true))
        unsortable_deferred = Administrate::Field::Deferred.
          new(double(sortable?: false))
